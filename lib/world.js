(function() {
  ...
  function World(role) {
    ...
    self.level.generate();
    _.each(self.level.obstacles, function(obstacle) {
  ...
  World.prototype.addUser = function(user) {
    ...
    self.level.tile(pos[0], pos[1], self.level.TILE_PLAYER);
    user.pos = pos;
    self.addObject(user);
  };
  ...
  World.prototype.removeUser = function(user) {
    var self = this;
    self.level.tile(user.pos[0], user.pos[1], self.level.TILE_EMPTY);
